...
include_once 'pw_imagebloc.php';

$home_slider = new Pw_ImageBloc();
$slides = [];
...
if (Tools::getValue('action') == 'updateSlidesPosition' && Tools::getValue('slides')) {
    ...
    foreach ($slides as $position => $id_slide) {
        $res = Db::getInstance()->execute('
			UPDATE `' . _DB_PREFIX_ . 'pwimagebloc_slides` SET `position` = ' . (int) $position . '
			WHERE `id_pwimagebloc_slides` = ' . (int) $id_slide
        );
    }
