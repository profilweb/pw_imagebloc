 ...
 */
class Pw_HomeBloc extends ObjectModel
{
    public $title;
    public $url;
    public $image;
    ...
    public static $definition = [
        'table' => 'pwimagebloc_slides',
        'primary' => 'id_pwimagebloc_slides',
        'multilang' => true,
        'fields' => [
    ...
    public function add($autodate = true, $null_values = false)
    {
        ...
        $res &= Db::getInstance()->execute('
			INSERT INTO `' . _DB_PREFIX_ . 'pwimagebloc` (`id_shop`, `id_pwimagebloc_slides`)
    ...
    public function delete()
    {
        ...
        $res &= Db::getInstance()->execute('
			DELETE FROM `' . _DB_PREFIX_ . 'pwimagebloc`
			WHERE `id_pwimagebloc_slides` = ' . (int) $this->id
        );
    ...
    public function reOrderPositions()
    {
        ...
        $rows = Db::getInstance((bool) _PS_USE_SQL_SLAVE_)->executeS('
			SELECT hss.`position` as position, hss.`id_pwimagebloc_slides` as id_slide
			FROM `' . _DB_PREFIX_ . 'pwimagebloc_slides` hss
			LEFT JOIN `' . _DB_PREFIX_ . 'pwimagebloc` hs ON (hss.`id_pwimagebloc_slides` = hs.`id_pwimagebloc_slides`)
			WHERE hs.`id_shop` = ' . (int) $id_shop . ' AND hss.`position` > ' . (int) $this->position
        );

        foreach ($rows as $row) {
            $current_slide = new Pw_HomeBloc($row['id_slide']);
